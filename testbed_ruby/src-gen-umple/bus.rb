...
class Bus
  ...
  #Bus Attributes - for documentation purposes
  #attr_reader :number

  #Bus Associations - for documentation purposes
  ...
  def add_seating(a_seating)
    ...
    existing_bus = a_seating.get_bus
    is_new_bus = (!existing_bus.nil? and !existing_bus.eql?(self))
    if is_new_bus
      a_seating.set_bus(self)
    else
      @seatings << a_seating
    end
    was_added = true
  ...
  def remove_seating(a_seating)
    was_removed = false
    # Unable to remove a_seating, as it must always have a bus
    unless a_seating.get_bus.eql?(self)

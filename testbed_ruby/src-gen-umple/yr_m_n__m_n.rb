...
class YR_m_n__m_n
  ...
  #YR_m_n__m_n Attributes - for documentation purposes
  #attr_reader :v

  #YR_m_n__m_n Associations - for documentation purposes
  #attr_reader :zVar
  ...
  def is_number_of_zVar_valid
    is_valid = number_of_zVar >= YR_m_n__m_n.minimum_number_of_zVar and number_of_zVar <= YR_m_n__m_n.maximum_number_of_zVar
    is_valid
  end
  ...
  def add_zVar(a_zVar)
    ...
    if number_of_zVar >= YR_m_n__m_n.maximum_number_of_zVar
      return was_added
    end

    @zVar << a_zVar
    if a_zVar.index_of_y_m_n__m_n(self) != -1
      was_added = true
    else
      was_added = a_zVar.add_y_m_n__m_n(self)
      unless was_added
        @zVar.delete(a_zVar)
      end
    end
  ...
  def remove_zVar(a_zVar)
    ...
    if number_of_zVar <= YR_m_n__m_n.minimum_number_of_zVar
      return was_removed
    end
    ...
    if a_zVar.index_of_y_m_n__m_n(self) == -1
      ...
      was_removed = a_zVar.remove_y_m_n__m_n(self)
  ...
  def set_zVar(new_zVar)
    ...
    if verified_zVar.size != new_zVar.length or verified_zVar.size < YR_m_n__m_n.minimum_number_of_zVar or verified_zVar.size() > YR_m_n__m_n.maximum_number_of_zVar
      return was_set
    end
    ...
    verified_zVar.each do |a_new_zVar|
      ...
      if old_zVar.include?(a_new_zVar)
        ...
        a_new_zVar.add_y_m_n__m_n(self)
      end
    end

    old_zVar.each do |an_old_zVar|
      an_old_zVar.remove_y_m_n__m_n(self)
    end
    was_set = true
    was_set
  end
